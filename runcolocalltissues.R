...
p2s = read.table("~/t2d/GTEx.v8.eGene.p_1.txt",stringsAsFactors=F,header=F)

#tissues = c("Whole_Blood")
for(tissue in tissues){
        print(tissue)
        setwd(tissue)
        p2 = p2s[which(p2s[,1] == tissue),4]

        print(p2)
        system("cp ../runcoloc_pertissue.R ./")
        system("cp ../coloc_per_block_pertissue.R ./")
        command = paste0("R --vanilla --args " , tissue, " ", p2, " < runcoloc_pertissue.R")
#        command = paste0("R --vanilla --args " , tissue, " 0.05 " < runcoloc_pertissue.R")
        system(command)
        print(command)
        setwd("../")
